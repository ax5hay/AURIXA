...
const PIE_COLORS = ["#3eb2a3", "#5b9ecf", "#8ba6b8", "#62c38f", "#e4ad5b", "#64748b"];

function EventVolumeTooltip(props: {
  active?: boolean;
  payload?: Array<{ payload: { name: string; count: number; latency: number; cost?: number } }>;
}) {
  ...
  return (
    <div
      className="px-4 py-3 rounded-xl border shadow-xl text-sm"
    ...
    >
      ...
      <p className="text-white/70">
        Events: <span className="text-aurixa-300 font-mono">{d.count?.toLocaleString()}</span>
      </p>
      <p className="text-white/70">
        Avg latency: <span className="font-mono">{d.latency}ms</span>
      </p>
      {d.cost != null && d.cost > 0 && (
        <p className="text-white/70">
          Cost: <span className="text-aurixa-400">${d.cost.toFixed(4)}</span>
        </p>
      )}
    </div>
  );
...
function ServiceTrafficTooltip(props: {
  active?: boolean;
  payload?: Array<{ payload: { name: string; events: number; cost?: number } }>;
}) {
  const { active, payload } = props;
  if (!active || !payload?.[0]?.payload) return null;
  const d = payload[0].payload;
  return (
    ...
    >
      ...
      <p className="text-white/70">
        Total events:{" "}
        <span className="text-aurixa-300 font-mono">{d.events?.toLocaleString()}</span>
      </p>
      {d.cost != null && d.cost > 0 && (
        <p className="text-white/70">
          Estimated cost: <span className="text-aurixa-400">${d.cost.toFixed(4)}</span>
        </p>
      )}
    </div>
  );
...
function EntityPieTooltip(props: {
  active?: boolean;
  ...
  return (
    ...
    >
      ...
      <p className="text-white/70">
        Count: <span className="text-aurixa-300 font-mono">{value.toLocaleString()}</span>
      </p>
...
export default function AnalyticsCharts({ overall, dbSummary, data }: AnalyticsChartsProps) {
  ...
  const avgLatency =
    totalCalls > 0
      ? Object.entries(overall).reduce(
          (s, [, m]) => s + (m?.avg_latency_ms || 0) * (m?.count || 0),
          0,
        ) / totalCalls
      : 0;
  ...
  const serviceChartData = data?.service_metrics
    ? Object.entries(data.service_metrics)
        .map(([svc, types]) => {
    ...
    <>
      {hasObservability && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <div className="glass rounded-xl p-4 border border-indigo-500/20 bg-indigo-500/5">
            <p className="text-[10px] font-semibold text-indigo-300/80 uppercase tracking-wider">
              Telemetry Events
            </p>
          ...
          <div className="glass rounded-xl p-4 border border-cyan-500/20 bg-cyan-500/5">
            <p className="text-[10px] font-semibold text-cyan-300/80 uppercase tracking-wider">
              Avg Latency
            </p>
          ...
          <div className="glass rounded-xl p-4 border border-amber-500/20 bg-amber-500/5">
            <p className="text-[10px] font-semibold text-amber-300/80 uppercase tracking-wider">
              LLM Cost
            </p>
          ...
          <div className="glass rounded-xl p-6 border border-white/5 overflow-hidden">
            <ResponsiveContainer width="100%" height={300}>
              ...
              >
                ...
                <Tooltip
                  content={<EventVolumeTooltip />}
                  cursor={{ fill: "rgba(62, 178, 163, 0.09)" }}
                />
                <Bar
                  dataKey="count"
          ...
          <div className="glass rounded-xl p-6 border border-white/5 overflow-hidden">
            <ResponsiveContainer width="100%" height={Math.max(220, serviceChartData.length * 48)}>
              <BarChart data={serviceChartData} layout="vertical" margin={{ left: 90, right: 16 }}>
                ...
                />
                <Tooltip
                  content={<ServiceTrafficTooltip />}
                  cursor={{ fill: "rgba(62, 178, 163, 0.07)" }}
                />
                <Bar
                  dataKey="events"
          ...
          <div className="glass rounded-xl p-6 border border-white/5 overflow-hidden">
            <div className="flex flex-col md:flex-row items-center gap-6">
              ...
              <div className="flex-1 text-sm text-white/50 space-y-1">
                <p>
                  Total entities:{" "}
                  <span className="text-white font-mono">{totalEntities.toLocaleString()}</span>
                </p>
                <p className="text-xs mt-2">
                  Breakdown of database records across core entity types used for analytics and RAG.
                </p>
              </div>
            </div>
          </div>
      ...
      {!hasObservability && (
        <div className="glass rounded-xl p-6 mb-8 border border-white/5 text-white/50 text-sm">
          Telemetry charts (event volume, service traffic) appear when the observability service
          receives events from pipelines.
        </div>
      )}
          ...
          <div className="glass rounded-xl overflow-hidden border border-white/5">
            <table className="w-full text-sm">
              <caption className="sr-only">
                Event counts, latency percentiles, and estimated cost by event type
              </caption>
              <thead>
                <tr className="border-b border-white/10">
                  <th scope="col" className="text-left py-3 px-4 font-medium text-white/60">
                    Type
                  </th>
                  <th scope="col" className="text-right py-3 px-4 font-medium text-white/60">
                    Count
                  </th>
                  <th scope="col" className="text-right py-3 px-4 font-medium text-white/60">
                    Avg (ms)
              ...
              <tbody>
                {Object.entries(overall).map(([type, m]) => (
                  <tr key={type} className="border-b border-white/5 hover:bg-white/[0.02]">
                    <th
                      scope="row"
                      className="py-3 px-4 text-left font-medium text-white/90 capitalize"
                    >
                      {type.replace(/_/g, " ")}
                    </th>
                    ...
                    <td className="py-3 px-4 text-right font-mono text-white/70">
                      {(m?.avg_latency_ms ?? 0).toFixed(0)}
                    </td>
                    <td className="py-3 px-4 text-right font-mono text-white/70">
                      {(m?.p95_latency_ms ?? 0).toFixed(0)}
                    </td>
