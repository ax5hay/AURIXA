...
import { getClient, type ClientProfile } from "../api";
import { CLIENT_SECTIONS } from "@/lib/client-sections";
...
export default function AccountPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      getClient(),
      fetch("/api/client/session", { cache: "no-store" }).then(async (response) => {
  ...
  return (
    <div className="space-y-9 py-4 sm:py-8">
        ...
        <div className="grid gap-5 lg:grid-cols-[1.1fr_.9fr]">
          <Card>
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-ui-muted">Client portal profile</p>
              </div>
            </div>
            <dl className="mt-6 divide-y divide-ui-border text-sm">
              <div className="grid gap-1 py-3 sm:grid-cols-[8rem_1fr]">
                ...
                <dd className="break-all text-ui-muted">{client.email || "Not provided"}</dd>
              </div>
              <div className="grid gap-1 py-3 sm:grid-cols-[8rem_1fr]">
                <dt className="font-semibold text-ui-ink">Phone</dt>
                <dd className="text-ui-muted">{client.phoneNumber || "Not provided"}</dd>
              </div>
            </dl>
            <Alert title="Need to correct this information?" tone="info" className="mt-5">
              Profile updates are not supported by the current client API. Contact your agent through
              a verified channel to make a correction.
            </Alert>
          </Card>
          ...
          <Card>
            ...
            <p className="text-sm leading-6 text-ui-muted">
              This browser session is client-scoped and expires automatically.
            </p>
            {session && (
              <dl className="mt-4 space-y-3 text-sm">
                ...
                {session.demo && (
                  <div>
                    ...
                    <dd className="mt-1 text-ui-muted">Not a production client identity.</dd>
                  </div>
                )}
              </dl>
            )}
      ...
      <section aria-label="Property and account tools">
        <SectionHeader
          title="More portal tools"
          description="Open a property area or review what is currently connected."
        />
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {[
            ...
            },
            {
              href: "/help",
              title: "Support",
              body: "Find agent guidance, fair housing notice, and support paths.",
            },
          ].map((item) => (
