...
export function Metric({
  ...
  return (
    <div className="border-l border-ui-border-strong pl-4 first:border-l-0 first:pl-0">
...
export function DataTable({
  ...
  return (
    <div
      className={clsx(
        "overflow-x-auto rounded-ui-lg border border-ui-border bg-ui-surface",
        className,
      )}
    >
      ...
      {items.map((item) => (
        <div
          ...
          className={clsx(
            ...
            item.urgent && "border-l-4 border-l-ui-danger",
          )}
        >
    ...
    <ol className="relative space-y-6 before:absolute before:bottom-3 before:left-[0.95rem] before:top-3 before:w-px before:bg-ui-border-strong">
      {items.map((item) => (
        <li key={item.id} className="relative grid grid-cols-[2rem_minmax(0,1fr)] gap-3">
          <span className="z-10 flex h-8 w-8 items-center justify-center rounded-full border border-ui-border-strong bg-ui-surface text-xs text-ui-accent">
            {item.icon ?? "•"}
          </span>
          <div className="pt-1">
