...
This document explains (1) how the **streaming-voice** service works, (2) how **streaming** fits into the channel layer, and (3) the **full layman flow** of the software for each type of user: AURIXA admin (operator), client, and agent/coordination staff.
...
1. **REST** — Used by the Client Portal Voice page (recommended, more reliable).
...
1. Request body: `audio_b64` (recorded audio as base64), optional `client_id` (or legacy `patient_id`), optional `want_tts` (default true).
...
4. **Pipeline**: Call the **orchestration engine** with that transcript (same as a chat message):  
   `POST http://orchestration-engine:8001/api/v1/pipelines` with `{ prompt: transcript, client_id }`.  
...
2. **Tenants**
   - See list of tenants (e.g. brokerages or property management organizations).
...
4. **Playground**
   ...
   - Execution actions: e.g. get_showings, get_client_financing, create_showing, create_service_request (execution engine + DB).
...
**Who:** A client (buyer, renter, seller, or owner) using the client-facing app.

**Where they go:** **Client Portal** — http://localhost:3300 (or your deployed URL).

**What they do:**

1. **Profile / home**
   - See their profile and a home screen.
   - Data loaded via API Gateway (e.g. client info, showings).

2. **Chat (text)**
   - Go to **Chat**.
   - Type a message → browser sends `POST /api/v1/orchestration/pipelines` with `{ prompt, client_id }`.
...
3. **Voice**
   ...
   - **Option A — Speak:**
     ...
     - Browser encodes the recording to base64 and sends **one** `POST /api/v1/voice/process` with `{ audio_b64, client_id, want_tts }`.
   ...
   - So for the client, “streaming” in practice is: one question (voice or text) → one answer (text + optional speech). The **streaming-voice service** is what does STT and TTS; the **channel** is REST (one request, one response per turn).

4. **Showings, listings, financing**
   - View showings, browse listings, check financing, submit applications or maintenance requests.
...
**End-to-end (layman):**  
Client opens Client Portal → chooses Chat or Voice.
...
### Agent / coordination staff

**Who:** Agents, brokers, coordinators, or operations staff for a brokerage (tenant) using the agent-facing app.

**Where they go:** **Agent Workspace** — http://localhost:3400 (or your deployed URL).

**What they do:**

1. **Identity**
   - “Logged in as” or role selection (e.g. agent, coordinator, operations).
   - Used for context in chat and for any tenant-scoped data.

2. **Chat (AI assistant)**
   - Staff use the in-app **Chat** to ask questions (e.g. policies, listings, client summaries).
   - Same mechanism as client chat: `POST /api/v1/orchestration/pipelines` with prompt (and tenant/user context if sent).
   - One request per message; one full text response. No voice or message streaming in the current design.

3. **Clients**
   - List and view clients (and client detail with showing history).
   - API calls go through the gateway to backend services that read from the database.

4. **Showings and leads**
   - View and manage showings; track leads through the pipeline.
...
**End-to-end (layman):**  
Staff open Agent Workspace → pick identity → use Chat, Clients, Showings, Leads, Schedule, Knowledge, Status. Every action is HTTP request → gateway → one or more backend services → response. No voice in the current Agent Workspace; no streaming except in the same sense as client chat (one message, one reply). The **streaming-voice** service is not in the path for staff unless you add a voice UI to the agent workspace later.
...
| Client         | Client Portal   | Chat (text), Voice (mic/text + play aloud), showings, listings, help | Yes (Voice tab)       | Voice: one REST request per turn (audio in/out); optional WebSocket for future |
| Agent staff    | Agent Workspace | Chat, clients, showings, leads, schedule, knowledge, status          | No                    | Same as chat: one request, one reply                                           |
...
- **Streaming in the channel layer**: Only the **voice WebSocket** is a real message stream (many messages back and forth). Voice REST and webchat are one request, one response per turn. LLM token streaming is available over the WebSocket voice path.
- **Admins** use the Dashboard for configuration, testing, and observability; no voice.
- **Clients** use the Client Portal for chat and voice; voice goes through the streaming-voice service (REST or WebSocket).
- **Agent staff** use the Agent Workspace for chat and operational screens; they do not use the streaming-voice service today.
