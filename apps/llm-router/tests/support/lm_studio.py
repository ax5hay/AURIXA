"""Shared helpers for llm-router live LM Studio integration tests."""
...
async def is_lm_studio_reachable() -> bool:
    base = host_lm_studio_base_url().rstrip("/").replace("/v1", "")
    try:
...
async def assert_lm_studio_available() -> dict[str, str]:
    ...
    return {"base_url": host_lm_studio_base_url(), "model": DEFAULT_MODEL}
